...
class HealthChecker:
    ...
    async def _check_replication_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            elif stats.get("failed_replications", 0) > stats.get("successful_replications", 0):
    ...
    async def _get_system_details(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "error": f"Error obteniendo detalles del sistema: {e}"
            }
    ...
    async def get_cached_health(self) -> Dict[str, Any]:
        ...
        if (self.last_update and 
            (datetime.utcnow() - self.last_update).total_seconds() < self.cache_ttl and
            self.health_cache):
    ...
    async def check_datanode_health(self, storage) -> Dict[str, Any]:
        ...
        try:
            ...
            if stored_chunks:
                ...
                chunk_integrity = (
                    "healthy" if await storage.verify_chunk_integrity(sample_chunk) 
                    else "degraded"
                )
...
async def check_external_service_health(url: str, timeout: float = 5.0) -> Dict[str, Any]:
    """
...
async def check_metadata_service_health(metadata_url: Optional[str] = None) -> Dict[str, Any]:
    """

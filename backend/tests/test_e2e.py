...
async def test_large_file(client):
    ...
    try:
        ...
        try:
            ...
            download_success = await client.download(
                remote_path, download_path, download_progress
            )
...
async def test_concurrent_uploads(client, temp_file):
    ...
    remote_paths = [
        f"/test/concurrent_{i}.bin" for i in range(3)
    ]  # Reducido para mayor velocidad

    # Upload concurrente
    tasks = [client.upload(temp_file, remote_path) for remote_path in remote_paths]
